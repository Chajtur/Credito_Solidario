...
$sql = $conn->prepare('select count(a.Numero_Prestamo) as cartera, sum(if(a.capital_mora > 0, 1, 0)) as morosos, sum(saldo_capital) as saldo, sum(capital_mora) as mora  
from prestamo a, agencia c 
where a.Estado_Credito = "Desembolsado" 
and get_agencia(a.gestor, a.agencia) = c.nombre
and c.idCoordinador = :user');
$sql->bindValue(':user', $user, PDO::PARAM_STR);
$sql->execute();
$cartasDeEstado = $sql->fetchAll();

/*query para obtener los para tabla y grafico*/
$sql = $conn->prepare('select a.agencia, count(a.Numero_Prestamo) as creditos, sum(a.Monto_Desembolsado) as desembolsado, sum(a.saldo_capital) as saldo, sum(a.capital_mora) as mora  
from prestamo a, agencia c where a.Estado_Credito = "Desembolsado"
and c.nombre = a.agencia
and c.idCoordinador = :user
group by a.agencia');
$sql->bindValue(':user', $user, PDO::PARAM_STR);
$sql->execute();
...
$sql = $conn->prepare('select count(a.Monto_Desembolsado) as ejecutado
from prestamo a, agencia b
where get_agencia(a.gestor, a.agencia) = b.nombre
and b.idCoordinador = :user
and Estado_Credito = "Desembolsado"
and year(Fecha_Desembolso) = year(current_date)');
$sql->bindValue(':user', $user, PDO::PARAM_STR);
$sql->execute();
$ejecutadoActual = $sql->fetchAll();

$promedioMensual = ($metaAnual[0]['metaCreditos'] - $ejecutadoActual[0]['ejecutado'])/(11 - date("n"));
                ...
                <div class="col s12 m8 l3 wow fadeInLeft" data-wow-offset="10" data-wow-duration="1.5s">
                    <div id="cardStat1" class="card">
                        <div class="card-content   white lighten-2 amber-text">
                            ...
                            <p class="card-stats-compare"><i class="material-icons">work</i> <?php echo number_format($promedioMensual); ?> meta mensual</p>
                        </div>
                ...
                <div class="col s12 m8 l3 wow fadeInRight" data-wow-offset="10" data-wow-duration="1.5s">
                    <div id="cardStat3" class="card wow fadeInDown" data-wow-duration="1s">
                        <div class="card-content white blue-text">
                            ...
                            <h4 id="porcentaje-mora" class="card-stats-number blue-text"><?php echo number_format($metaAnual[0]['metaCreditos']); ?> Créditos</h4>
                            <p class="card-stats-compare"><i class="material-icons">work</i> <?php echo number_format($ejecutadoActual[0]['ejecutado']); ?> en el año</p>
                        </div>
