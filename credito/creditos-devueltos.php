...
try{
    ...
    from cartera_consolidada where Estatus_Prestamo = "Para Correccion" and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')');
    $stat->execute();
    
    $grupos_recibidos = $stat->fetchAll();
    
    $stat = $conn->prepare('select distinct gestor from cartera_consolidada where Estatus_Prestamo = "Para Correccion" and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')');
    $stat->execute();
            ...
            <div class="card">
                <div class="card-content">
                    <span class="card-title">
                        <div class="row">
                            <div class="col l8">Créditos Devueltos</div>
                            <div class="col l4">
                            </div>
                        </div>
                    </span>
                                                ...
                                                <li>
                                                    <div class="collapsible-header sin-icon">
                                                        <div class="col s12 l12">
                                                            <center>
                                                                <h5>No hay créditos devueltos.</h5>
                                                            </center>
...
$(document).ready(function () {
    ...
    $('#breadcrum-title').text('Créditos Devueltos');
    ...
    $('#recibidos-success').click(function(){
        ...
        if(window.grupos.length == 0){
            swal("Ningún Crédito Seleccionado", "Por favor seleccione los créditos que recibió de regreso.", "error");
            return false;
        }
            
        swal({
            title: "¿Se le entregaron los creditos seleccionados?",
            text: "Al precionar el boton de Confirmar, los grupos serán recibidos de nuevo",
            type: "warning",
        ...
        function(){
            $.ajax({
                type: 'POST',
                url: '../php/credito/recibir-grupo.php',
