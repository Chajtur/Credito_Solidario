                                        ...
                                        <?php if(count($documentos) > 0):?>
                                            ...
                                            <?php foreach($documentos as $documento):?>
                                               ...
                                               <?php $date = new DateTime($documento['fecha_log']);?>
                                                ...
                                                <?php 
                                                    $arr = explode('.', $documento['nombre_documento']);
                                                    $ext = end($arr);
                                                ?>
                                                
                                                <li>
                                                    <div class="collapsible-header sin-icon">
                                                        <div class="col s6 l7 nombreg truncate"><a href="../docs/<?php echo ($ext == 'xls' ? 'excel/' : '');?><?php echo $documento['nombre_documento'];?>" target="_blank"><?php echo $documento['nombre_documento'];?></a></div>
                                                        <div class="col s4 l3 fechag truncate"><?php echo $date->format('d/m/Y');?></div>
                                                        <div class="col s2 l2 horag truncate"><?php echo $date->format('H:i:s');?></div>
                                                    </div>
                                                </li>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#select-filtro-mes').on('change', function(){
            ...
            $('#buscarHas').val($('#select-filtro-mes option:selected').val());
            window.listObj.fuzzySearch($('#select-filtro-mes option:selected').val());
        ...
        var options = {
            page: 10,
            pagination: true,
            valueNames: [ 'nombreg', 'fechag', 'horag' ],
            fuzzySearch: {
