    ...
    try{
        ...
        foreach($creditos as $hash){
            ...
            while($result = $stat_creditos->fetch(PDO::FETCH_ASSOC)){
                agregarEnBitacora($stat_bitacora, $result['id'], $hash, 'Recibido despues de devolucion', 'Devuelto', $_SESSION['user'], 'Recibido en créditos');
            }
        ...
        $stat_bitacora = $conn->prepare('select id_usuario from bitacora_creditos where grupo_hash = :hash and estado_credito = "Para Correccion" order by fecha desc limit 1');
        ...
        $nombre = 'RECEPCION_CREDITOS_DEVUELTOS_'.$_SESSION['user'].'_'.$date.'.pdf';
        
        // Desde aca se genera el archivo
        $archivo = new ReceptionReport($nombre, 'Créditos', $_SESSION['user']);
