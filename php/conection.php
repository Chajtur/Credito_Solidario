...
try{
    ...
    if(!isset($conn))
        $conn = new PDO('mysql:host=181.210.15.138;dbname=credito_solidario','credito_solidario_usr','12345678', array(
    ...
    if(!isset($conn2))
        $conn2 = new PDO('mysql:host=181.210.15.138;dbname=larahrm','credito_solidario_usr','12345678', array(
            PDO::ATTR_PERSISTENT => true //El hecho que las conexiones sean persistentes provoca que haya un aumento en la velocidad de las consultas, pero puede existir una sobrecarga en el servidor de MySql
        ));
    if(!isset($conn3))
