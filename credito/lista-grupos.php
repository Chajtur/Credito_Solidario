...
$stat = $conn->prepare('select distinct grupo_solidario_hash, Grupo_Solidario, gestor, Estatus_Prestamo, ciclo 
    from cartera_consolidada where Estatus_Prestamo in ("Ingresado", "Para Control de Calidad", "Para Coordinacion", "Para Digitacion") and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')');
$stat->execute();
$grupos = $stat->fetchAll();

$stat = $conn->prepare('select identidad, nombre from cartera_consolidada 
    where Estatus_Prestamo in ("Ingresado", "Para Control de Calidad", "Para Coordinacion", "Para Digitacion") and grupo_solidario_hash = :hash');
