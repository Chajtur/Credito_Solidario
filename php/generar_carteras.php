...
try{
    ...
    $sql = $conn->prepare('call obtener_cartera_afectada()'); //<- llamar al procedimiento para realizar la consulta
    $sql->execute();
    ...
    //cartera antigua
    $archivo->createSheet(2);
    $archivo->setActiveSheetIndex(2);
    
    $sql = $conn->prepare('call obtener_cartera_antigua()'); //<- llamar al procedimiento para realizar la consulta
    $sql->execute();
    ...
    if($datos = $sql->fetchAll()){
        ...
        agregarFormulas($archivo, $cantidad, $letters); // <- agregando formulas a la hoja activa
        darEstilo($archivo, $cantidad); // <- darle estilo a la hoja activa
        $archivo->getActiveSheet()->setTitle('CARTERA ANTIGUA'); //<-asignarle nombre a la hoja activa
    ...
    // cartera nuevos recursos -------------------
    
    $archivo->createSheet(3);
    $archivo->setActiveSheetIndex(3);
    
    $sql = $conn->prepare('call cartera_nuevos_recursos()'); //<- llamar al procedimiento para realizar la consulta
    $sql->execute();
    ...
    if($datos = $sql->fetchAll()){
        ...
        agregarFormulas($archivo, $cantidad, $letters); // <- agregando formulas a la hoja activa
        darEstilo($archivo, $cantidad); // <- darle estilo a la hoja activa
        $archivo->getActiveSheet()->setTitle('CARTERA NUEVOS RECURSOS'); //<-asignarle nombre a la hoja activa
    ...
    //cartera oficial
    $archivo->createSheet(4);
    $archivo->setActiveSheetIndex(4);
    
    $sql = $conn->prepare('call obtener_cartera_oficial()'); //<- llamar al procedimiento para realizar la consulta
    $sql->execute();
    ...
    if($datos = $sql->fetchAll()){
        ...
        agregarFormulas($archivo, $cantidad, $letters); // <- agregando formulas a la hoja activa
        darEstilo($archivo, $cantidad); // <- darle estilo a la hoja activa
        $archivo->getActiveSheet()->setTitle('CARTERA OFICIAL'); //<-asignarle nombre a la hoja activa
...
function agregarFormulas($excel, $cant, $letras){
    ...
    $porcentajes = [
        'I3:I'.$cant,'J3:J'.$cant,'P3:P'.$cant,'Q3:Q'.$cant,'W3:W'.$cant,'X3:X'.$cant,'AD3:AD'.$cant,'AE3:AE'.$cant,'AK3:AK'.$cant,'AL3:AL'.$cant,'AR3:AR'.$cant,'AS3:AS'.$cant,
    ];
    
    foreach($porcentajes as $col){
        
        $excel->getActiveSheet()
            ->getStyle($col)->getNumberFormat()->applyFromArray(
                array( 
