...
try{
    ...
    from cartera_consolidada where Estatus_Prestamo = "Ingresado" and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')';
    $stat = $conn->prepare($sql);
    $stat->execute();
    
    $grupos_recibidos = $stat->fetchAll();
    
    $stat = $conn->prepare('select distinct gestor from cartera_consolidada where Estatus_Prestamo = "Ingresado" and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')');
    $stat->execute();
            ...
            <div class="card">
                <div class="card-content">
                    <span class="card-title">
                        <div class="row">
                            <div class="col l8">Recuperación de Créditos</div>
                            <div class="col l4">
                            </div>
                        </div>
                    </span>
                        ...
                        <form id="form-grupos-recibidos">
                            ...
                            <div class="row">
                                <div class="input-field col s12 l6 offset-l3">
                                    <a id="recibidos-success" href="#!" class="btn waves-effect waves-light col s12 ">Recuperar Créditos</a>
                                </div>
                            </div>
                        </form>
                    </div>
...
$(document).ready(function () {
    ...
    $('#breadcrum-title').text('Recuperación de Créditos');
    ...
    $('#recibidos-success').click(function(){
        ...
        if(window.grupos.length == 0){
            swal("Ningún Crédito Seleccionado", "Por favor seleccione los créditos para recuperarlos", "error");
            return false;
        }
            
        swal({
            title: "¿Está seguro/a que desea recuperar los créditos seleccionados?",
            text: "Al precionar el boton de Confirmar, los grupos pasarán a estado devuelto",
            type: "warning",
        ...
        function(){
            ...
            $.ajax({
                type: 'POST',
                url: '../php/credito/recuperar-grupo.php',
                ...
                success: function(data){
                    ...
                    if(data == 'true'){
                        ...
                        swal({
                            title: "Completado", 
                            text: "Los créditos se han recuperado correctamente.", 
                            type: "success"
                        }, function(){
                        ...
                        swal('Error desconocido','No se ha procesado la solicitud','error');
