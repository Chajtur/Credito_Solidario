...
try{
    ...
    session_start();
    $stat = $conn->prepare('select distinct grupo_solidario_hash, Grupo_Solidario, gestor, Estatus_Prestamo, ciclo, programa
    ...
    $stat = $conn->prepare('select distinct gestor from cartera_consolidada where Estatus_Prestamo = "Devuelto" and agencia in ('.'"'.implode('","', $_SESSION['agencia']).'"'.')');
    $stat->execute();
                                                ...
                                                <?php foreach($grupos_beneficiarios as $grupo):?>
                                                    ...
                                                    <li id="collapsible<?php echo $grupo['hash'];?>">
                                                        <div class="collapsible-header sin-icon" id="header">
                                                            ...
                                                            <input type="hidden" id="programa" value="<?php echo $grupo['programa'];?>">
                                                        </div>
                                                        <div class="collapsible-body no-padding-custom">
                                                            <br>
                                                            <div class="row margin">
                                                                <div class="col l10 m10 s10 offset-l1">
                                                                    <h5 class="light">Beneficiarios
                                                                        <a href="#!" class="right btn-anular-grupo" id=""><i class="material-icons red-text">backspace</i></a>
                                                                    </h5>
                                                                </div>
...
$(document).ready(function () {
    ...
    $('#breadcrum-title').text('Re-ingreso de Créditos');
    ...
    $('#recibidos-success').click(function(){
        ...
        $.each(window.grupos, function(index, value){
            ...
            var cantidadBeneficiariosMinimos = 2;

            if(value.cantidad < cantidadBeneficiariosMinimos && value.programa == "P01"){
        ...
        function(){
            $.ajax({
                type: 'POST',
                url: '../php/coordinador-digitador/reingreso-grupo.php',
    ...
    $('.btn-anular-grupo').click(function(){

        swal({
            title: "¿Está seguro que desea anular el crédito?",
            text: "Esta acción será registrada en la bitácora.",
            type: "warning",
            showCancelButton: true,
            closeOnConfirm: false,
            showLoaderOnConfirm: true
        ...
        function(){
            var obj = {
                hash: window.current.group
            }
            $.ajax({
                type: 'POST',
                url: '../php/credito/anular-credito.php',
                data: 'data='+JSON.stringify(obj),
                success: function(data){
                    if(data == true){
                        $('#floating-refresh').trigger('click');
                        swal("Anulado", "El crédito ha sido anulado","success");
                    }else{
...
function createEmptyObjectElement(){
    ...
    empty.programa = $('#collapsible'+window.current.group).find('#programa').val();
    return empty;
}
</script>