...
if(isset($_POST['data'])){
    ...
    try{
            ...
            if($grupo->verificado){ //Si el grupo ha sido verificado, se procesa
                ...
                foreach($grupo->beneficiarios as $beneficiario){
                    ...
                    if(sizeof($beneficiario->checklist) > 0){ // Si es mayor a cero, entonces tiene observaciones (checkboxes)
                        ...
                        agregarEnBitacora($stat_bitacora, $beneficiario->id_credito, $grupo->hash, 'Correccion Call Center', 'Call Center', $_SESSION['user'], 'Correccion Call Center');
                        // agregarEnBitacora($stat_bitacora, $beneficiario->id_credito, $grupo->hash, 'Devuelto con Correcciones', 'Para Correccion', $_SESSION['user'], 'Devuelto con Correcciones'); en caso de que se vuelva a requerir devolver desde call center, descomentar esta línea y eliminar la anterior
                ...
                */
                ...
                $stat_estatus->bindValue(':estado', 'Para Control de Calidad', PDO::PARAM_STR);
                // $stat_estatus->bindValue(':estado', ($correcto ? 'Para Control de Calidad' : 'Para Correccion'), PDO::PARAM_STR); en caso de que se vuelva a requerir devolver desde call center, descomentar esta línea y eliminar la anterior
        ...
        // si la bandera es verdadera hacemos commit 
        $conn->commit();
