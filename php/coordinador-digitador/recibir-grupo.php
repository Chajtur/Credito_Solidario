...
if(isset($_POST['credito'])){
    ...
    try{
        ...
        $stat = $conn->prepare('update cartera_consolidada set Estatus_Prestamo = "Digitacion" where grupo_solidario_hash = :hash');
        ...
        foreach($creditos as $hash){
            ...
            while($result = $stat_creditos->fetch(PDO::FETCH_ASSOC)){
                agregarEnBitacora($stat_bitacora, $result['id'], $hash, 'Recibido Digitacion', 'Digitacion', $_SESSION['user'], 'Recibido Digitacion');
            }
        ...
        $stat_bitacora = $conn->prepare('select id_usuario from bitacora_creditos where grupo_hash = :hash and estado_credito = "Coordinacion" order by fecha desc limit 1');
        ...
        $nombre = 'RECEPCION_DIGITACION_'.$_SESSION['user'].'_'.$date.'.pdf';
        
        // Desde aca se genera el archivo
        $archivo = new ReceptionReport($nombre, 'Digitacion', $_SESSION['user']);
