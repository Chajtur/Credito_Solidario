...
try{
    ...
    from cartera_consolidada where Estatus_Prestamo = "Para Control de Calidad"');
    $stat->execute();
    
    $grupos_recibidos = $stat->fetchAll();
    
    $stat = $conn->prepare('select distinct gestor from cartera_consolidada where Estatus_Prestamo = "Para Control de Calidad"');
    $stat->execute();
