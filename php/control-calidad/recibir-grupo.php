...
if(isset($_POST['credito'])){
    ...
    try{
        ...
        $stat = $conn->prepare('update cartera_consolidada set Estatus_Prestamo = "Control de Calidad" where grupo_solidario_hash = :hash');
        ...
        foreach($creditos as $hash){
            ...
            while($result = $stat_creditos->fetch(PDO::FETCH_ASSOC)){
                agregarEnBitacora($stat_bitacora, $result['id'], $hash, 'Recibido Control de Calidad', 'Control de Calidad', $_SESSION['user'], 'Recibido Control de Calidad');
            }
        ...
        $stat_bitacora = $conn->prepare('select id_usuario from bitacora_creditos where grupo_hash = :hash and estado_credito = "Call Center" order by fecha desc limit 1');
        ...
        $nombre = 'RECEPCION_CONTROL_CALIDAD_'.$_SESSION['user'].'_'.$date.'.pdf';
        
        // Desde aca se genera el archivo
        $archivo = new ReceptionReport($nombre, 'Control de Calidad', $_SESSION['user']);
