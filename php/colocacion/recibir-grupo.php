...
if(isset($_POST['credito'])){
    ...
    try{
        ...
        $stat = $conn->prepare('update cartera_consolidada set Estatus_Prestamo = "Colocacion" where grupo_solidario_hash = :hash');
        ...
        foreach($creditos as $hash){
            ...
            while($result = $stat_creditos->fetch(PDO::FETCH_ASSOC)){
                agregarEnBitacora($stat_bitacora, $result['id'], $hash, 'Recibido Colocacion', 'Colocacion', $_SESSION['user'], 'Recibido Colocacion');
            }
        ...
        $stat_bitacora = $conn->prepare('select id_usuario from bitacora_creditos where grupo_hash = :hash and estado_credito = "Digitacion" order by fecha desc limit 1');
        ...
        $nombre = 'RECEPCION_COLOCACION_'.$_SESSION['user'].'_'.$date.'.pdf';
        
        // Desde aca se genera el archivo
        $archivo = new ReceptionReport($nombre, 'Colocación', $_SESSION['user']);
